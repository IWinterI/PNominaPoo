# Skill Registry — Nomina

Generated: 2026-05-15
Source: sdd-init (SDD initialization)

## User Skills (opencode)

| Skill | Description | Triggers |
|-------|-------------|----------|
| branch-pr | PR creation workflow for Agent Teams Lite following the issue-first enforcement system | Creating a pull request, opening a PR, preparing changes for review |
| issue-creation | Issue creation workflow for Agent Teams Lite following the issue-first enforcement system | Creating a GitHub issue, reporting a bug, requesting a feature |
| judgment-day | Parallel adversarial review protocol that launches two independent blind judge sub-agents | "judgment day", "judgment-day", "review adversarial", "dual review", "doble review", "juzgar", "que lo juzguen" |
| go-testing | Go testing patterns for Gentleman.Dots, including Bubbletea TUI testing | Writing Go tests, using teatest, adding test coverage |
| skill-creator | Creates new AI agent skills following the Agent Skills spec | Creating a new skill, adding agent instructions, documenting patterns for AI |

## Project-Level Skills

None found.

## Project Conventions

No project-level agent configuration files found (AGENTS.md, CLAUD.md, .cursorrules, GEMINI.md, copilot-instructions.md).

Note: AGENTS.md exists at `/home/luis/.config/opencode/AGENTS.md` (user-level).
